<x-filament-panels::page>
                ...
                <x-filament::input.select wire:model.live="versionId">
                    @forelse ($versions as $version)
                        <option value="{{ $version->id }}">
                            {{ $version->valid_from->toFormattedDateString() }}
                            @if($version->valid_to) &ndash; {{ $version->valid_to->toFormattedDateString() }} @endif
    ...
    @if ($versionId)
        <div style="margin-top:1.5rem;overflow-x:auto">
            <table style="width:100%;border-collapse:collapse;font-size:0.875rem">
                <thead>
                    <tr>
                        <th style="border:1px solid rgb(226 232 240 / 1);padding:0.5rem;text-align:left;width:4rem">Period</th>
                        @foreach ($days as $day)
                            ...
                            @foreach ($days as $day)
                                ...
                                <td style="border:1px solid rgb(226 232 240 / 1);padding:0.5rem;vertical-align:top;{{ $slot?->is_break ? 'background-color:rgb(249 250 251 / 1)' : '' }}">
                                    ...
                                    @elseif ($slot->is_break)
                                        <span style="font-size:0.75rem;color:rgb(156 163 175 / 1)">Break</span>
    ...
    @else
        <p style="margin-top:1.5rem;color:rgb(107 114 128 / 1)">No timetable version exists yet for this teacher. Create one above.</p>
    @endif
</x-filament-panels::page>
